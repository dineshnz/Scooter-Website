...
?>
<?php
	$result = mysql_query('select * from asset where assetName like "%'. $_POST['assetName'].'%"');
?>
...
<div id="search">
  ...
  <form class="submit" action="list.php" method="post">
...
<div id="container">
    ...
    <div class="left">
		...
		<div>
       <form action="asset_select.php"  method="get" style="width:238px; background-color: #f8f8f8; border: 1px solid  #f8f8f8;">
	
		Condition: 
					<p><select name="condition">
						<option name="condition" value="1" >TimeStamp</option>
						<option name="condition" value="2" >Price</option>
					</select></p>
		Sort Order:
					<p><select name="order">
						<option name="order" value="1">asc</option>
						<option name="order" value="2">desc</option>
					</select></p>
					<br/>
...
?>
		...
		<div class="button">
			<strong><a href="showCart.php?assetID=<?=$row['assetID'] ?>&assetName=<?=$row['assetName']?>&price=<?=$row['price']?>&img=<?=$row['img']?>&make=<?=$row['make']?>&timestamp=<?=$row['timestamp']?>" style="color:white">Add to Cart</a></strong>
		</div>
